...
procedure Tmain.ExchangeIcons;                        {Show status of copy action}
begin
        ...
        if length(inlist[1])>2 then begin
          if data then begin                           {Index column header found, now all following lines are waveform data}
            ...
            if pos(idIndex, inlist[i])>0 then begin    {Header of data found}
              lblY.Caption:=rsVolt+tab1+kop+EinheitStr(Einheit(inlist[i].Split([sep])[1]))+kcl;
...
procedure Tmain.mnCopyClick(Sender: TObject);
begin
  gbCopy.Visible:=true;
end;
